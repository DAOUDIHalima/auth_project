...
from .form import DossierMedicalForm
def dossier_medical_detail(request):
    dossiers = DossierMedical.objects.all()
    return render(request, 'dossier/dossier.html', {'dossiers': dossiers})


def create_dossier_medical(request):
    if request.method == 'POST':
        form = DossierMedicalForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('create_info')  # Rediriger vers la liste des visites après l'enregistrement
        else:
            print("Form errors:", form.errors)  # Debug print to check form errors
    else:
        form = DossierMedicalForm()

    return render(request, 'dossier/create_dossier_medical.html', {'form': form})

def update_dossier_medical(request, pk):
    dossier = get_object_or_404(DossierMedical, pk=pk)
    if request.method == 'POST':
        form = DossierMedicalForm(request.POST or None, request.FILES or None, instance=dossier)
        if form.is_valid():
            form.save()
            return redirect('dossiers')
    else:
        form = DossierMedicalForm(instance=dossier)
    return render(request, 'dossier/update_dossier_medical.html', {'form': form, 'dossier': dossier})

def delete_dossier_medical(request, pk):
    dossier = get_object_or_404(DossierMedical,pk=pk)
    dossier.delete()
    return redirect('dossiers')
...
def create_info(request):
    ...
    return render(request, 'info/create_info.html', {'form': form})

def update_info(request, pk):
    inf = get_object_or_404(info, pk=pk)
    if request.method == 'POST':
        form = InfoForm(request.POST, instance=inf)
        if form.is_valid():
            form.save()
            return redirect('infos')
    else:
        form = InfoForm(instance=inf)
    return render(request, 'info/update_info.html', {'form': form, 'inf': inf})
...
def view_info(request, pk):
    inf = get_object_or_404(info, pk=pk)
    form = InfoForm(instance=inf)
    return render(request, 'info/view_info.html', {'form': form, 'inf': inf}) 

from django.shortcuts import render, get_object_or_404, redirect
from .models import systematique
from .form import SystematiqueForm  # Import your form class here

def systematique_detail(request):
    systematiques = systematique.objects.all()
    return render(request, 'systematique/systematique.html', {'systematiques': systematiques})

def create_systematique(request):
    if request.method == 'POST':
        form = SystematiqueForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('systematiques')
        else:
            print("Form errors:", form.errors)
    else:
        form = SystematiqueForm()

    return render(request, 'systematique/create_systematique.html', {'form': form})

def update_systematique(request, pk):
    sys_info = get_object_or_404(systematique, pk=pk)
    if request.method == 'POST':
        form = SystematiqueForm(request.POST, instance=sys_info)
        if form.is_valid():
            form.save()
            return redirect('systematiques')
    else:
        form = SystematiqueForm(instance=sys_info)
    return render(request, 'systematique/update_systematique.html', {'form': form, 'sys_info': sys_info})

def delete_systematique(request, pk):
    sys_info = get_object_or_404(systematique, pk=pk)
    if request.method == 'POST':
        sys_info.delete()
        return redirect('systematiques')

def view_systematique(request, pk):
    sys_info = get_object_or_404(systematique, pk=pk)
    form = SystematiqueForm(instance=sys_info)
    return render(request, 'systematique/view_systematique.html', {'form': form, 'sys_info': sys_info})
